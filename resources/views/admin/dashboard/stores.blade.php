...
<section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1> {{_i('text.admin.stores.select_store_title')}}</h1>
            </div>
          </div>
        </div><!-- /.container-fluid -->
</section>
              ...
              <div class="card bg-light">
                  <div v-if="store.is_active" class="ribbon-wrapper">
                        <div class="ribbon bg-success">
                            {{_i('text.admin.stores.open')}}
                              </div>
                  </div>
                  <div v-if="!store.is_active" class="ribbon-wrapper">
                        <div class="ribbon bg-danger">
                            {{_i('text.admin.stores.close')}}
                              </div>
                  </div>
                <div class="card-header text-muted border-bottom-0">
                    {{_i('text.admin.stores.store_name')}} :
                </div>
                <div class="card-body pt-0">
                  <div class="row">
                    <div class="col-7">
                      <h2 class="lead"><b>@{{store.name}} </b></h2>
                      <p class="text-muted text-sm"><b>{{_i('admin.layout.title')}}: </b> @{{store.description}} </p>
                      <ul class="ml-4 mb-0 fa-ul text-muted">
                      <li class="small"><span class="fa-li"><i class="fas fa-lg fa-building"></i></span> ///</li>
                      <li class="small"><span class="fa-li"><i class="fas fa-lg fa-phone"></i></span>  {{_i('text.admin.stores.created_at')}} #: @{{store.updated_at}}</li>
                      </ul>
                    </div>
                ...
                <div class="card-footer">
                  <div class="text-right">
                    <button v-bind:disabled="!store.is_active" class="btn btn-sm btn-primary">
                        <ion-icon name="appstore"></ion-icon> <a class="btn btn-sm" v-bind:href="store.controlPanelUrl"> {{_i('text.admin.stores.select_store')}}<a>
                    </button>
                  </div>
                </div>
              </div>
